...
#currently only works for 3 features + intercept, need to generalise so it works with more features

X, y, coefs = make_regression(n_samples = 100,  n_features = 3, n_informative = 3, noise = 5, coef = True)
...
def f(x, z, b_hat):
	  return b_hat[0] + b_hat[1]*x + b_hat[1]*z
...
ax.contour3D(x_flat, z_flat, y_hat, 50, cmap='binary')
